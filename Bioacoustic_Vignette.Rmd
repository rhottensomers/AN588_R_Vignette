...
</audio>') #this is html code I found online to embed mp3 files(https://community.rstudio.com/t/audio-files-in-r-markdown/20874)
```
```
...
TD
```
As you can see this has identified fewer events compared to blob detection(the exact number, actually). See how the extracted data compares between the data frames. Because of how each form of detection filters audio, one type may be better to use than the other.
...
?auto_detec
detection <- auto_detec(threshold = 5,
ssmooth = 300, #'smoothens' spectrogram
bp = c(2, 9), #lower and upper limits of a frequency bandpass filter (in kHz)
